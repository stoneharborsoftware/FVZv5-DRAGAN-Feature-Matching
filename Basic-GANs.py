...
# Define Basic-GAN and it's components 
class Discriminator(tf.keras.Model):
    def __init__(self, input_win, output_win):
        ...
        self.l7 = ReLU()
        self.l8 = Dense(1, activation='sigmoid')
...
class Basic_GAN(object):
    ...
    def train(self, segment_train, y_target, historical_price):
        ...
        for step in range(1):
            ...
            with tf.GradientTape() as disc_tape:                
                ...
                loss_real = self.cross_entropy(tf.ones_like(output_real), output_real)
                loss_fake = self.cross_entropy(tf.zeros_like(output_fake), output_fake)           
        ...
        for step in range(3):
            with tf.GradientTape() as gen_tape:                
                ...
                # y_target = tf.expand_dims(y_train, axis=2)
                # X_real =  tf.concat([historical_price, y_target], axis=1)                
                gen_loss = self.cross_entropy(tf.ones_like(output_fake), output_fake) 
    ...
    def test(self, segment_test):
        ...
        # plt.show()
        # exit(0)
        #self.gen.load_weights('Basic_GAN_145.h5')
...
if __name__ == '__main__':
    ...
    # Define Basic-GAN and initialize
    feature_m = segment_train.shape[2]
    Basic_GAN_model = Basic_GAN(input_win, output_win, feature_m)
    ...
    for iteration in range(iterations):
        dis_loss_, gen_loss_, y_predict = Basic_GAN_model.train(segment_train, y_train, h_prices_train)
